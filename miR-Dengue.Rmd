...
#normalizacao
```{r}
CTs %>% rownames_to_column(., var="miRNA") %>% reshape2::melt(id_vars='miRNA') %>% group_by(miRNA) %>% rstatix::get_summary_stats(show=c('ci', 'sd')) %>% View()
...
Avg.CT <- data.frame(Grave = round((rowMeans(CTs[,1:2])),2),
                      Con_signo = round((rowMeans(CTs[,3:4])),2),
...
amostras$Condition <- c(rep("Grave", 2), rep("Signo de Alarma", 2), rep("Sin Signo de Alarma", 2))
