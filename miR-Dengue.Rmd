...
dCT[29,]  #Verificar posição do miR normalizador
...
amostras$Condition <- c(rep("Grave", 3), rep("Signo de Alarma", 2), rep("Sin Signo de Alarma", 2))
...
dend %>% 
  ...
  color_labels(labels = colnames(dCT.2[4:5]) , col=cores[2]) %>%
  color_labels(labels = colnames(dCT.2[6:7]), col=cores[3]) %>%
  plot() 
legend("topright",
...
##Heatmap expressao
```{r}
pheatmap(dCT[-29,], #Remover miR usado como normalizador (todos seus valores serão = 0)
         border_color = NA,
         cluster_cols = F,
         color=colorRampPalette(c("blue", "black", "yellow"))(100),
