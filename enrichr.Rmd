...
latam <- latam.ori %>% dplyr::select(Year, Country_sp, TIS, PopFemale, PopMale, Population, Muertes, Total.Casos.de.dengue, Incidence.per.100.000.pop.)
...
latam %>% filter(Country_sp %in% latam.top$Country_sp) %>% ggplot(aes(Country_sp, Total.Casos.de.dengue, fill=Year))+
  stat_summary(fun='mean', geom='bar', position='dodge')+
  scale_x_discrete(limit=latam.top$Country_sp)+
  ggbreak::scale_y_cut(breaks=c(900000, 8000000), scales = c(0,2), expand = T, which = c(0,2))
```
